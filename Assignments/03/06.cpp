...
int main(int argc, char* argv[]) {
	...
	// Queue up a large number of tasks- in this case, an arbitrary 10 since that seems to be the
	// maximum number of threads that a web browser can use, and also a reasonable average of cores
	// that most machines on the recieving end will have.
	for (auto i = 0; i < 10; i++) {
		tasks.push_back(std::move(std::async(std::launch::async, task)));
	}
	
	// Run all 10 tasks so that they're ready to recieve requests.
	for (auto i = 0; i < 10; i++) {
		tasks[i].get();
	}
}

...
int main(int argc, char* argv[]) {
	...
	auto task = [&]() {
		...
		// Create a task that can be queued up and run asynchronously
		while (connection) {
	...
	// Queue up a large number of tasks- in this case, an arbitrary 300 since we have roughly 300 images to retrieve.
	for (auto i = 0; i < 300; i++) {
		tasks.push_back(std::move(std::async(std::launch::async, task)));
	}
	
	// Some assumptions and notes about what calling tasks[i].get() on all 300 tasks will do:
	// - Blue has 20 or 40 threads, but probably won't allocate that many.
	// - Most of these tasks will sit waiting until some other task running on a thread has to wait for data.
	// - 300 is certainly overkill, but knowing that the system will manage its own resources, I don't think
	//	 there's a downside to going overboard here.
	// - The maximum CPU usage I saw was 1.8%, which I'm guessing was only on one core.
	for (auto i = 0; i < 300; i++) {
		tasks[i].get();
	}
}
